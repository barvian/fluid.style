...
<Steps.Root>
  ...
  <Steps.Item>
    **Use `rem` for `screens` and `fontSize`**
    ...
    Use `rem` versions of Tailwind's default screens and font sizes/line heights
    to ensure greater compability with core plugins. Make sure any custom screens or font sizes
    you add also use `rem` units.
    ...
    <Fragment slot="code">
      ```ts title="tailwind.config.ts" ins=/, defaultThemeScreensInRems, defaultThemeFontSizeInRems/ ins={10,15}
