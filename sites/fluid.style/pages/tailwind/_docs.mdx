...
<Steps.Root>
  ...
  <Steps.Item>
    ...
    <Fragment slot="code">
      ```ts title="tailwind.config.ts" ins=/, defaultThemeScreensInRems/ ins={11} mark=/\/\/.*$/
      import { fluidExtractor, fluidCorePlugins, defaultThemeScreensInRems } from '@fluidstyle/tailwindcss'
...
<Resizer client:load height="h-32">
  <button class="bg-sky-500 hover:bg-sky-700 ~@lg/2xl:~px-4/8 ~@lg/2xl:~py-2/4 ~@lg/2xl:~text-sm/xl rounded-full font-semibold text-white">Fluid button</button>
</Resizer>
```html mark=/(\~.*?)\s+/
