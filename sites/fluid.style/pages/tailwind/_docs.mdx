...
<Resizer client:load height="h-32">
  <button class="bg-sky-500 hover:bg-sky-700 ~@lg/2xl:~px-4/8 ~@lg/2xl:~py-2/4 ~@lg/2xl:~text-sm/xl leading-[1.25] rounded-full font-semibold text-white">Fluid button</button>
</Resizer>
```html mark=/(\~.*?)\s+/
...
(Admittedly, `lg:~lg:` is pretty repetitive, so it may try to be simplified in the future.)
{/* 
