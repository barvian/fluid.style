...
<Steps.Root>
  <Steps.Item>
    **Install the package**
    ...
    Install `@fluidstyle/tailwindcss` via npm.
    <Fragment slot="code">
    ```sh
    npm install -D @fluidstyle/tailwindcss
    ```
    </Fragment>
  </Steps.Item>
  <Steps.Item>
    **Add the [extractor](https://tailwindcss.com/docs/content-configuration#customizing-extraction-logic)**
    ...
    <Fragment slot="code">
      ```ts title="tailwind.config.ts" ins={1, 6} ins=/content\:(\s*\{)/ ins=/\](,)/ ins=/files\:\s*/ ins=/(\}),/
      import { fluidExtractor } from '@fluidstyle/tailwindcss'
  ...
  <Steps.Item>
    **Add the fluid core plugins**
    ...
    <Fragment slot="code">
      ...
      import { fluidExtractor, fluidCorePlugins } from '@fluidstyle/tailwindcss'
  ...
  <Steps.Item>
    **Use `rem` for `screens` and `fontSize`**

    Make sure your `theme.screens` and `theme.fontSize` (including line heights) use `rem`
    units. If you're using Tailwind's default breakpoints or font sizes, the plugin exports
    rem versions for your convenience.
    <Tip.Info>This [may be unnecessary](https://github.com/tailwindlabs/tailwindcss/discussions/8378#discussioncomment-2779675) in future versions of Tailwind</Tip.Info>
    <Fragment slot="code">
      ```ts title="tailwind.config.ts" mark=/, defaultThemeScreensInRems, defaultThemeFontSizeInRems/ mark={10,11,14,15}
      import { fluidExtractor, fluidCorePlugins, defaultThemeScreensInRems, defaultThemeFontSizeInRems } from '@fluidstyle/tailwindcss'
      ...
      export default {
        ...
        theme: {
          fontSize: {
            ...defaultThemeFontSizeInRems, // if you're using Tailwind's default font sizes
            '10xl': ['10rem', '10rem'] // make sure custom font sizes/line heights use rem units
          },
          screens: {
            xs: '30rem', // make sure custom screens use rem units
            ...defaultThemeScreensInRems // if you're using Tailwind's default screens
          }
        },
        plugins: [
          fluidCorePlugins
...
* Fluid utilities interpolate between their start value and end value when the viewport is between the start point and end point, respectively
* The start and end points default to the smallest and largest screen, but [they can be configured globally](#customizing-default-screens) or per-utility (below)
...
Due to CSS restrictions, fluid expressions can only be computed if **all involved lengths have the same units**.
This includes:
* Start value / end value
* Start point / end point

This usually isn't an issue, especially if you extend the `defaultThemeScreensInRems` object
as mentioned in the [installation instructions](#installation) but it can be helpful
...
There's currently no way to reference a [named container](https://github.com/tailwindlabs/tailwindcss-container-queries#named-containers)
when using fluid CSS expressions, but
[browser support should come soon](https://github.com/w3c/csswg-drafts/issues/7858).
...
<AnnotatedCode>
  ...
  import { ..., type FluidConfig } from '@fluidstyle/tailwindcss'
...
import { ..., fluidize } from '@fluidstyle/tailwindcss'
...
{/* 
```html del="lg:~lg:" ins="~:lg:" del="min-[40rem]:~min-[40rem]/xl:" ins="~/xl:min-[40rem]:" del="@lg:~@lg:" ins="~:@lg:"
<!-- These are equivalent: -->
<h1 class="lg:~lg:~text-4xl/base">Whoa!</h1>
<h1 class="~:lg:~text-4xl/base">Whoa!</h1>

<h1 class="min-[40rem]:~min-[40rem]/xl:~text-4xl/base">Whoa!</h1>
<h1 class="~/xl:min-[40rem]:~text-4xl/base">Whoa!</h1>
